...
import { MessageSquare, Eye, Pin, MapPin, ArrowRight } from "lucide-react";
                ...
                <CardContent className="p-0">
                    <div className="divide-y">
                        {forumTopics.map((topic) => {
                            const lastReply = topic.replies && topic.replies.length > 0 ? [...topic.replies].sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())[0] : null;
                            return (
                                <div key={topic.topicId} className="p-4 hover:bg-secondary/50 transition-colors">
                                    ...
                                    {lastReply && (
                                        <div className="mt-3 pl-14 text-sm text-muted-foreground flex items-center gap-2">
                                            <Avatar className="w-5 h-5 border">
                                                <AvatarImage src={lastReply.userPhoto} alt={lastReply.userName} />
                                                <AvatarFallback>{lastReply.userName.charAt(0)}</AvatarFallback>
                                            </Avatar>
                                            <p className="truncate"><span className="font-medium text-foreground">{lastReply.userName}:</span> "{lastReply.content}"</p>
                                            <Link href={`/forums/${topic.topicId}`} className="ml-auto text-primary font-semibold text-xs whitespace-nowrap hover:underline flex items-center gap-1">
                                                View replies <ArrowRight className="w-3 h-3"/>
                                            </Link>
                                        </div>
                                    )}
                                </div>
